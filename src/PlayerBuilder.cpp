...
void PlayerBuilder::RemoveShip(Pos pos) {
    ...
    for (std::size_t i = 0; i < GRID_SIZE; i++)
        for (std::size_t j = 0; j < GRID_SIZE; j++)
            if (_shipData[i][j])
                InsertShipMargin(*_shipData[i][j]);
...
static PlayerBuilder::ShipOrientation RandomOrientation() {
    ...
    std::uniform_int_distribution<std::underlying_type_t<Ort>> direction(
        std::to_underlying(Ort::Horizontal),
        std::to_underlying(Ort::Vertical)
    );
    
    return static_cast<Ort>(direction(mt));
}

PlayerBuilder::ShipData PlayerBuilder::CreateShip(Pos start, ShipSize size, ShipOrientation ort) noexcept {
    Pos end;
...
bool PlayerBuilder::ValidateShipPlacement(const ShipData& ship) const noexcept {
    // overflow
    if (ship.end.x > GRID_SIZE || ship.end.y > GRID_SIZE)
        return false;
